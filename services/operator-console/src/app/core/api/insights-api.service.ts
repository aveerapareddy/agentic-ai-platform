...
import type { CrossExecutionInsightDto, MuktiInsightsSummaryDto } from '../models/insights.models';
import type { ApiErrorBody } from '../models/execution.models';

export interface MuktiInsightsQuery {
  tenant_id?: string;
...
export class InsightsApiService {
  ...
  getMuktiInsights(query: MuktiInsightsQuery): Observable<MuktiInsightsSummaryDto> {
    ...
    return this.http
      .get<MuktiInsightsSummaryDto>(this.url('/v1/insights/mukti'), { params })
      .pipe(catchError((e) => this.mapError(e)));
  }

  getMuktiInsight(insightId: string, query: MuktiInsightsQuery): Observable<CrossExecutionInsightDto> {
    let params = new HttpParams();
    if (query.tenant_id) params = params.set('tenant_id', query.tenant_id);
    if (query.workflow_type) params = params.set('workflow_type', query.workflow_type);
    if (query.status) params = params.set('status', query.status);
    if (query.limit != null) params = params.set('limit', String(query.limit));
    return this.http
      .get<CrossExecutionInsightDto>(this.url(`/v1/insights/mukti/${insightId}`), { params })
      .pipe(catchError((e) => this.mapError(e)));
  }
