...
def scope_key(input_payload: dict[str, Any]) -> str:
    return str(
        input_payload.get("scope_id")
        or input_payload.get("incident_id")
        or input_payload.get("id")
        or "unknown",
    )


def simulate_latency(input_payload: dict[str, Any], *, base_ms: int = 15, cap_ms: int = 120) -> None:
    """Deterministic short delay from scope/incident id (no randomness)."""
    raw = scope_key(input_payload)
...
def incident_key(input_payload: dict[str, Any]) -> str:
    return scope_key(input_payload)
